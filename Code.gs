...
/* Cuatro páginas: la interna del equipo (Index), la que firma el huésped
   (Ficha, con ?f=<token>), la del equipo de aseo (Aseo, con ?aseo=<clave>) y
   el editor del plano (Plano, con ?plano=1). Las tres últimas no piden
   clave. */
function doGet(e) {
  var p = (e && e.parameter) || {};
  var pagina, titulo;
  ...
  if (p.f) {
    ...
    // El editor del plano de la planta, para el trámite sanitario. Es una
    // página suelta: no lee ni escribe en la planilla, guarda el trabajo en
    // el mismo navegador y se comparte con el enlace terminado en ?plano=1.
    pagina = HtmlService.createTemplateFromFile('Plano');
    titulo = 'Casona Peumayén — Plano';
  } else if (p.aseo) {
